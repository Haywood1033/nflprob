...
const { recentGames } = require('./recency-window.js');
// QB anchor is rushing-only — a starting QB's receiving TDs are effectively zero (trick
// plays aside), so there's no receiving leg for this position, unlike RB. tdPerCarry is
// higher than RB's because QB carries skew toward goal-line sneaks and read-option keepers,
// which convert at a much higher rate than the average rushing attempt. Estimated league
// average, not empirically fit — needs the same real-season calibration the rest of this
// model is still waiting on.
const LG = {
...
function computePlayerUsage(rows, playerName, throughWeek, lastN = 5) {
  const candidateRows = rows.filter(r => r.player_display_name === playerName && r.season_type === 'REG');
...
const LG_TDS_ALLOWED = { WR: 0.9, TE: 0.35, RB: 0.55, QB: 0.20 };
...
function anytimeTdProb(usage, defAllowed, teamImplied, ctx = {}) {
  ...
  if (pos === 'RB') {
    ...
    // Rushing only — see the LG.QB comment above.
    const carriesPerGame = usage.carries / usage.games;
    const tdPerCarry = regress(usage.rushTds / Math.max(usage.carries, 1), anchors.tdPerCarry, usage.carries, anchors.k);
    p = carriesPerGame > 0 ? 1 - Math.pow(1 - tdPerCarry, carriesPerGame) : 0;
  } else {
    const targetsPerGame = usage.targets / usage.games;
...
function getTdSignals(usage, defAllowed, teamModel) {
  const isRusher = usage.position === 'RB' || usage.position === 'QB';
  const signals = [
    { key: 'usage',   active: usage.position === 'RB' ? (usage.carries / usage.games) >= 12
                            : usage.position === 'QB' ? (usage.carries / usage.games) >= 4
                            : (usage.targets / usage.games) >= 6, label: 'High Usage' },
    ...
    { key: 'role',    active: isRusher ? usage.rushTds > 0 : usage.wopr >= 0.5, label: 'Featured Role' },
