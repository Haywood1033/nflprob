...
// teamFilter (nflverse abbr, e.g. 'PHI') is REQUIRED, not optional. Without it, a player
// who was traded mid-season (rows under two different teams) gets the same cross-team
// aggregate applied to BOTH teams' candidate pools — e.g. Tank Bigsby (JAX week 1, traded
// to PHI week 3+) showed up as a "top rusher" for both JAX and PHI with identical carries,
// because the last-5-game window was computed once from his name alone and reused for
// both team contexts. Scoping to the team being queried fixes this at the source.
