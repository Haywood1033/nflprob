...
async function fetchPlayerWeekStats(season) {
  ...
  // Same reasoning as lib/nflverse.js's fetchTeamWeekStats: don't accept a season's file
  // just because the HTTP request succeeded — verify it has real REG-season rows first,
  // so an early/empty/preseason-only 2026 file doesn't silently block the 2025 fallback.
  for (const trySeason of [season, season - 1]) {
    try {
      ...
      if (!hasRegRows) {
        console.warn(`nflverse player-week file for ${trySeason} exists but has no REG rows yet — trying older season`);
        continue;
      }
