...
// regardless of why.
//
// EXPANDED: this only ever checked injuries[0].status === 'out', which misses the
// `injuredReserveOrOut` roster group entirely — a player on season-ending IR often has no
// injuries[0].status of literally "Out" (it reads "Injured Reserve", "IR", "PUP", etc., or
// the injuries array can be empty while the group itself says injuredReserveOrOut). Real
// symptom: a QB done for the season on IR kept outranking the actual current starter in
// candidate pools (season-long cumulative volume from before the injury still beat the new
// starter's smaller sample), because this check silently let him through as available.
// Anyone ESPN buckets into that group is definitionally not playing, so the group itself is
// now a hard signal, independent of the exact wording of the injuries entry.
const DEFINITELY_OUT_STATUSES = new Set([
  'out', 'ir', 'injured reserve', 'reserve/injured', 'pup', 'physically unable to perform',
  'nfi', 'non-football injury', 'reserve/covid-19', 'reserve/suspended', 'suspended', 'exempt',
]);
function isDefinitelyOut(p, statusGroup) {
  if (statusGroup === 'injuredReserveOrOut') return true;
  const injuryStatus = (p.injuries?.[0]?.status || '').trim().toLowerCase();
  return DEFINITELY_OUT_STATUSES.has(injuryStatus);
}
    ...
    for (const g of groups) {
      ...
      for (const p of (g.items || [])) {
        ...
        roster[normalizeName(p.displayName)] = {
          ...
          definitelyOut: isDefinitelyOut(p, g.position),
